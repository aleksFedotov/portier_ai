...
SYSTEM_PROMPT = (
    ...
    "(акты, УПД, счета-фактуры) — тоже guest_message. Подтверждение бронирования "
    "TravelLine — это booking_confirmed, даже если в нём есть «Способ оплаты: "
    "Безналичный расчет», «К оплате на расчетный счет» и реквизиты плательщика "
    "в блоке «Детали платежа»: счёт по такому письму не запрашивается\n"
    ...
    "Для booking_confirmed также заполни guests_count — сколько гостей проживает "
    "по брони (целое число; если в письме не указано — null).\n"
