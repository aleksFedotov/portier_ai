...
class Settings(BaseSettings):
    ...
    OWNER_NOTICE_RULES: list[str] = [
        ...
        # счёт KDV — поставщик, мы им счета не выставляем (только они нам):
        # любой ящик домена, тема со «счет»/«счёт» → просто сообщение
        # владельцу, в группу счетов и в invoice_required не отправляем
        "@kdvonline.ru|счет",
        "@kdvonline.ru|счёт",
    ]
    ...
    LOGIN_CODE_RULES: list[str] = [
        ...
        # Google: оповещения безопасности и коды входа (решение владельца — как
        # login_code, в группу счетов)
    ...
    MUTED_SENDERS: list[str] = [
        ...
        # Броневик: «Просьба выставить счет за услугу #…» — счёт по ним не
        # выставляем (решение владельца 31.08.2026). С этого же адреса приходят
        # заявки агентов — они нужны, поэтому глушим только по шаблону темы.
        "billing@bronevik.online|просьба выставить счет",
